...
# Simple requests    
    ...
    def erase_one(self, key_tool):
        key_tool = str(key_tool)
        sql = """
        DELETE FROM tool WHERE key = '""" + key_tool + """';
        """
        self.execution(self.db_adr, sql)
    ...
    def erase_tool(self, tool):
        ...
        if len(res) > 0:
            ...
            for i in selection_list:
                counter = counter + 1
                self.speak_dialog('speak.erase.list',{'counter': counter, 'i': i})
            nr_from_sel_list = self.get_response('speak.erase')
            if nr_from_sel_list == None:
                return
